...
import { PAGE_SIZES, dataUnggahDiskon, optionsGedung, optionsKota, optionsTahunAjaran } from "@/components/go-kasir/data";
import Selects from "@/components/Ui/Selects";
import Button from "@/components/Ui/Button";
import DialogModalConfirmation from "@/components/Modal/ModalConfirmation";
import { optionsTingkatKelas } from "@/data/smba/data";

const UnggahDokumenDiskon = () => {
    ...
    const sortedRecords = useMemo(() => sortBy(dataUnggahDiskon, "namaKaryawan"), []);
    ...
    useEffect(() => {
        ...
        setRecords(dataUnggahDiskon.slice(from, to));
    }, [page, pageSize]);

    useEffect(() => {
        const data = sortBy(dataUnggahDiskon, sortStatus.columnAccessor);
                    ...
                    columns={[
                        {
                            ...
                            title: "No",
                            width: 75,
                            render: (number) => dataUnggahDiskon.indexOf(number) + 1,
                        },
                        ...
                        items: (record) => [
                            {
                                ...
                                title: `Ubah ${record.namaSiswa}`,
                                onClick: () => handleUbah(),
                            },
                        ],
                    }}
                    totalRecords={dataUnggahDiskon.length}
...
export default UnggahDokumenDiskon;
